...
class ModalCriarEvento(discord.ui.Modal, title="Criar Evento"):
    nome = discord.ui.TextInput(label="Nome do evento", placeholder="ex: Top 1 da semana", max_length=50)
    descricao = discord.ui.TextInput(label="Descrição", placeholder="ex: Quem ganhar mais partidas leva R$50", max_length=200)
    premio = discord.ui.TextInput(label="Prêmio", placeholder="ex: R$ 50,00 ou cargo VIP", max_length=100)
    ...
    async def on_submit(self, interaction):
        ...
        data["eventos"].append({
            "nome": self.nome.value, "descricao": self.descricao.value,
            "premio": self.premio.value, "ativo": True,
            "criado_em": datetime.now().isoformat()
        })
        db.save(interaction.guild.id, data)
        await interaction.response.send_message("✅ Evento **" + self.nome.value + "** criado!", ephemeral=True)
...
class ViewEventos(discord.ui.View):
    ...
    async def config_geral(self, interaction, button):
        await interaction.response.send_message("Em breve: configurações gerais de eventos!", ephemeral=True)
